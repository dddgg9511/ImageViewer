#pragma once
#define DISALLOW_COPY_AND_ASSIGN(CLine) \
	CLine(const CLine&); \
	void operator=(const CLine&);
class IAnnotation;
class CLine : public IAnnotation
{
public:
	CLine(const PointF& downPoint,const PointF& upPoint,const Color& penColor,
				const float& penWidth);
	CLine(const AnnotationData& annotationData);
